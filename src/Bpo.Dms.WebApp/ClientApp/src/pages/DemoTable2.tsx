...
import { getProducts, postProducts, patchProducts, getProductsBySearch, getProductsAction, deleteProductsById } from '@/services/dms/Products';
...
const EditableTable = () => {
  ...
  const deleteProduct = async (record: any) => {
    ...
    if (result) {
      try {
        await deleteProductsById(record.id);
        message.success(`Xóa sản phẩm ${record.name} thành công`);
      } catch (e: any) {
        message.error('Lỗi');
      } finally {
        var dataProduct: API.Product[] = [];
        await getProducts().then((data: any) => data.forEach((product: any) => dataProduct.push(product)))
        setData(dataProduct);
      }
    }
  }
  ...
  //   try {
  //       const data = await getProducts1(value)
  //       setData(data)
  //     // }
  //   } catch (err) {
  //     console.log('Lỗi: ', err);
  ...
  const onChange = async (pagination: any, filters: any, sorter: any, extra: any) => {
    ...
    const dataA = await getProductsAction(filters.name, sort, pagination.current, pagination.pageSize);
